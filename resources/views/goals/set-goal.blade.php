...
<script>
    document.title = "StudentWell | Set New Goal";
</script>
@if (session('success'))
    ...
    <div class="container">
        ...
        <div class="mb-3">
            <a href="{{ route('goals.index') }}" class="back-link h2 fw-bold">
                <i class="fa fa-chevron-left me-2 h4 fw-bold"></i>Set New Goal
            </a>
        </div>

        <!-- Set New Goal Form -->
        <div class="custom-form-container p-4">
            <form method="POST" action="{{ route('goals.store.set') }}">
                @csrf
                <div class="row">
                        ...
                        <div class="left-column-wrapper d-flex flex-column h-100">
                                ...
                                <select class="form-select custom-input @error('GoalCategory') is-invalid @enderror"
                                    ...
                                    @foreach ($goalCategories as $category => $emoji)
                                        <option value="{{ $category }}"
                                            {{ old('GoalCategory') == $category ? 'selected' : '' }}>
                                            {{ $emoji }} {{ $category }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3 mt-1 w-50 pe-4 mobile-full-width">
                                ...
                                <input type="date"
                                    ...
                                    id="GoalStartDate" name="GoalStartDate" value="{{ old('GoalStartDate') }}"
                                    min="{{ $today }}">
                                @error('GoalStartDate')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="mb-3 mt-1 w-50 pe-4 mobile-full-width">
                                ...
                                <input type="date"
                                    ...
                                    id="GoalTargetDate" name="GoalTargetDate" value="{{ old('GoalTargetDate') }}"
                                    min="{{ old('GoalStartDate') ?? $today }}">
                    ...
                    <div class="col-lg-6">
                        <div class="content-wrapper">
                            ...
                            <textarea class="form-control custom-input content-area-height @error('Notes') is-invalid @enderror" id="Notes"
                                name="Notes" placeholder="Any other details about your goal...">{{ old('Notes') }}</textarea>
                            @error('Notes')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
