...
<script>
    document.title = "StudentWell | Edit Exercise";
</script>
@if (session('success'))
    ...
    <div class="container">
        ...
        <div class="mb-3">
            <a href="{{ route('exercise.index') }}" class="back-link h2 fw-bold">
                <i class="fa fa-chevron-left me-2 h4 fw-bold"></i>Edit Exercise
            </a>
        </div>

        <!-- Edit Exercise Form -->
        <div class="custom-form-container p-4">
            <form method="POST" action="{{ route('exercise.update', $exercisePlan->PlannedExerciseID) }}">
                @csrf
                @method('PUT')
                <div class="row">
                    ...
                    <div class="col-lg-6">
                        <div class="content-wrapper">
                            ...
                            <textarea class="form-control custom-input content-area-height @error('Notes') is-invalid @enderror" id="Notes"
                                name="Notes" placeholder="Write your exercise notes here...">{{ old('Notes', $exercisePlan->Notes ?? '') }}</textarea>
                            @error('Notes')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
